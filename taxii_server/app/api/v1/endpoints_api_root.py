...
async def list_collections_in_api_root(api_root_path: str, db: Session = Depends(get_db), current_user: bfore_types.User = Depends(get_current_user)):
    ...
    for coll in db_collections:
        can_read = coll.is_public_readable or (current_user and current_user.is_superuser)
        if current_user and current_user.role and current_user.role.name in ["admin", "consumer", "publisher"]: can_read = True
        if can_read:
            can_write = False
            if current_user and (current_user.is_superuser or (current_user.role and current_user.role.name in ["admin", "publisher"])): can_write = True
            collections_resp.append(taxii_schemas.Collection(
                ...
                can_read=can_read, can_write=can_write, media_types=coll.media_types
            ))
