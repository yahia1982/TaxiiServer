...
def get_collection_or_404(
    db: Session, api_root_id: uuid.UUID, collection_id_str: str
) -> CollectionModel:
    try:
        collection_uuid = uuid.UUID(collection_id_str)
    except ValueError:
        raise HTTPException(
            status_code=404, detail=f"Invalid Collection ID format: {collection_id_str}"
        )
    collection = crud_taxii.collection["get"](
        db, collection_id=collection_uuid, api_root_id=api_root_id
    )
    if not collection:
        raise HTTPException(
            status_code=404,
            detail=f"Collection '{collection_id_str}' not found in API Root '{api_root_id}'",
        )
    return collection


@router.get(
    "/{api_root_path}/collections/{collection_id}/",
    response_model=taxii_schemas.Collection,
...
async def get_collection_info(
    ...
    return taxii_schemas.Collection(
        ...
        alias=db_collection.alias,
        can_read=True,
        can_write=can_write,
        media_types=db_collection.media_types,
    )
