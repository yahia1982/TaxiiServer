...
@router.get(
    "/{api_root_path}/collections/{collection_id}/manifest/",
    response_model=taxii_schemas.Manifest,
    summary="Get Collection Manifest",
    tags=["Manifests"],
)
async def get_collection_manifest(
    ...
    if match_version == "last":
        raise HTTPException(
            ...
            detail="match[version]=last for manifests is not yet implemented.",
        )
    offset = int(next_val) if next_val and next_val.isdigit() else 0
    db_records = crud_taxii.manifest_record["get_multi_from_collection"](
        db,
    ...
    for rec in db_records:
        date_added = (
            rec.date_added_to_collection.replace(tzinfo=timezone.utc)
            if rec.date_added_to_collection.tzinfo is None
            else rec.date_added_to_collection
        )
        version_ts = (
            rec.stix_modified_timestamp.replace(tzinfo=timezone.utc)
            if rec.stix_modified_timestamp.tzinfo is None
            else rec.stix_modified_timestamp
        )
        manifest_records_resp.append(
            taxii_schemas.ManifestRecord(
                id=rec.stix_id,
                date_added=date_added,
                version=version_ts.isoformat().replace("+00:00", "Z"),
                media_type=rec.media_type,
            )
        )
    next_page_url = None
    if limit and len(db_records) == limit:
        next_page_url = str(request.url.replace_query_params(next=str(offset + limit)))
    return taxii_schemas.Manifest(
        objects=manifest_records_resp, more=bool(next_page_url), next=next_page_url
    )
