...
async def get_collection_manifest(api_root_path: str, collection_id: str, request: Request, db: Session = Depends(get_db), current_user: UserModel = Depends(get_current_active_user), added_after: Optional[datetime] = Query(None), match_id: Optional[str] = Query(None, alias="match[id]"), match_type: Optional[str] = Query(None, alias="match[type]"), match_version: Optional[str] = Query(None, alias="match[version]"), limit: Optional[int] = Query(None, ge=1), next_val: Optional[str] = Query(None, alias="next")):
    ...
    db_records = crud_taxii.manifest_record["get_multi_from_collection"](db, collection_id=db_collection.id, added_after=added_after, stix_ids=stix_ids, stix_types=stix_types, stix_versions_dt=stix_versions_dt, limit=limit, offset=offset)
