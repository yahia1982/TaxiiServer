...
async def get_objects_from_collection(api_root_path: str, collection_id: str, request: Request, db: Session = Depends(get_db), current_user: bfore_types.User = Depends(get_current_user), added_after: Optional[datetime] = Query(None), match_id: Optional[str] = Query(None, alias="match[id]"), match_type: Optional[str] = Query(None, alias="match[type]"), match_version: Optional[str] = Query(None, alias="match[version]"), limit: Optional[int] = Query(None, ge=1), next_val: Optional[str] = Query(None, alias="next")):
    ...
    if match_version == "last" and stix_ids and len(stix_ids) == 1:
        latest_versions = crud_taxii.stored_object["get_all_versions_by_id"](db, collection_id=db_collection.id, stix_id=stix_ids[0], user_scopes=current_user.scopes)
        if latest_versions: db_objects_orm = [latest_versions[0]]
    else:
        db_objects_orm = crud_taxii.stored_object["get_multi_from_collection"](db, collection_id=db_collection.id, added_after=added_after, stix_ids=stix_ids, stix_types=stix_types, stix_versions_dt=stix_versions_dt, limit=limit, offset=offset, user_scopes=current_user.scopes)
...
async def add_objects_to_collection(api_root_path: str, collection_id: str, bundle: taxii_schemas.STIXBundle = Body(...), db: Session = Depends(get_db), current_user: bfore_types.User = Depends(get_current_user)):
    db_api_root = get_api_root_or_404(db, api_root_path)
    db_collection = get_collection_or_404(db, db_api_root.id, collection_id)
...
async def get_object_by_stix_id(api_root_path: str, collection_id: str, object_id: str, db: Session = Depends(get_db), current_user: bfore_types.User = Depends(get_current_user), match_version: Optional[str] = Query(None, alias="match[version]")):
    db_api_root = get_api_root_or_404(db, api_root_path)
    db_collection = get_collection_or_404(db, db_api_root.id, collection_id)
    check_read_permission(db_collection, current_user)
    db_objects_orm = []
    if match_version == "last":
        all_versions = crud_taxii.stored_object["get_all_versions_by_id"](db, collection_id=db_collection.id, stix_id=object_id, user_scopes=current_user.scopes)
        ...
        db_objects_orm = crud_taxii.stored_object["get_all_versions_by_id"](db, collection_id=db_collection.id, stix_id=object_id, user_scopes=current_user.scopes)
