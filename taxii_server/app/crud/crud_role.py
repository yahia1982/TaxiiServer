...
def update_role(db: Session, role_obj: Role, role_in: Union[RoleUpdate, Dict[str, Any]]) -> Optional[Role]:
    ...
    for field, value in update_data.items(): setattr(role_obj, field, value)
    db.add(role_obj); db.commit(); db.refresh(role_obj)
    return role_obj

def delete_role(db: Session, role_id: uuid.UUID) -> Optional[Role]:
    role_obj = db.query(Role).get(role_id) # Changed to .get() for primary key lookup
    if role_obj: db.delete(role_obj); db.commit()
    return role_obj

role_crud = { "get": get_role, "get_by_name": get_role_by_name, "get_multi": get_roles, "create": create_role, "update": update_role, "delete": delete_role }
