...
class Collection(Base):
    ...
    media_types = Column(
        JSONB, nullable=False, default=lambda: ["application/stix+json;version=2.1"]
    )
    api_root_id = Column(
        UUID(as_uuid=True), ForeignKey("api_roots.id"), nullable=False, index=True
    )  # Added index
    api_root = relationship("ApiRoot", back_populates="collections")
    objects = relationship(
        "StoredObject", back_populates="collection", cascade="all, delete-orphan"
    )
    manifest_records = relationship(
        "ManifestRecord", back_populates="collection", cascade="all, delete-orphan"
    )
...
class StoredObject(Base):
    ...
    stix_modified_timestamp = Column(
...
class ManifestRecord(Base):
    ...
    media_type = Column(
        String(255), nullable=False, default="application/stix+json;version=2.1"
    )
    date_added_to_collection = Column(
        DateTime(timezone=True), server_default=func.now(), index=True
    )  # Already indexed
    collection_id = Column(
        UUID(as_uuid=True), ForeignKey("collections.id"), nullable=False, index=True
